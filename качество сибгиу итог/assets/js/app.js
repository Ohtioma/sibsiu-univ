...
function getDeviceId() {
  let id = localStorage.getItem('sibgiu_device_id');
  if (!id) {
    id = uid() + uid().replace('_', '');
    localStorage.setItem('sibgiu_device_id', id);
  }
  return id;
}
function hasDeviceUsedLink(linkId) {
  const link = Store.surveyLinks.find(l => l.id === linkId);
  if (!link) return false;
  return (link.usedDeviceIds || []).includes(getDeviceId());
}
function markDeviceUsedLink(linkId) {
  const links = Store.surveyLinks;
  const link = links.find(l => l.id === linkId);
  if (!link) return;
  if (!link.usedDeviceIds) link.usedDeviceIds = [];
  const deviceId = getDeviceId();
  if (!link.usedDeviceIds.includes(deviceId)) {
    link.usedDeviceIds.push(deviceId);
    Store.surveyLinks = links;
  }
}
function showSurveyLinkMessage(title, subtitle) {
  ...
  overlay.innerHTML = `
    <div style="flex:1;display:flex;align-items:center;justify-content:center;padding:20px">
      <div style="background:#fff;border-radius:16px;padding:40px;text-align:center;max-width:320px;box-shadow:0 8px 40px rgba(21,101,192,.13)">
        <div style="font-size:18px;font-weight:700;color:#212121;margin-bottom:8px">${escHtml(title)}</div>
        <div style="font-size:14px;color:#9e9e9e">${escHtml(subtitle)}</div>
      </div>
    </div>`;
}
function _slug(name) {
...
let _surveySearch          = '';
let _selectedAnketaIds     = new Set();
...
function countSurveyRespondents(surveyId) {
  return Store.surveyLinks
    .filter(l => l.surveyId === surveyId)
...
function renderDashboard() {
  ...
  cont.innerHTML = '<div class="survey-cards">' + surveys.map(s => {
    ...
    const respCount = countSurveyRespondents(s.id);
...
function renderSurveyOpros(s) {
  ...
  const anketaRows = (s.anketaIds || []).map(aid => {
    ...
    const checked = _selectedAnketaIds.has(aid) ? 'checked' : '';
    return `<li class="cl-row">
      <input type="checkbox" ${checked} onchange="toggleAnketaCheck('${aid}', this.checked)" style="margin-right:8px;width:15px;height:15px;flex-shrink:0;cursor:pointer">
  ...
  const programRows = (s.programIds || []).map(pid => {
    ...
    const checked = _selectedProgramIds.has(pid) ? 'checked' : '';
    return `<li class="cl-row">
      <input type="checkbox" ${checked} onchange="toggleProgramCheck('${pid}', this.checked)" style="margin-right:8px;width:15px;height:15px;flex-shrink:0;cursor:pointer">
  ...
  document.getElementById('survey-opros').innerHTML = `
    ...
    <div class="section-block">
      <div class="section-head">
        ...
        <div style="display:flex;gap:8px">
          <button id="bulk-remove-anketa" class="btn btn-sm" style="border:1.5px solid #e53935;color:#e53935;${_selectedAnketaIds.size ? '' : 'display:none'}" onclick="bulkRemoveAnkety()">Убрать выбранные (${_selectedAnketaIds.size})</button>
    ...
    <div class="section-block">
      <div class="section-head">
        ...
        <div style="display:flex;gap:8px">
          <button id="bulk-remove-prog" class="btn btn-sm" style="border:1.5px solid #e53935;color:#e53935;${_selectedProgramIds.size ? '' : 'display:none'}" onclick="bulkRemovePrograms()">Убрать выбранные (${_selectedProgramIds.size})</button>
...
function removeAnketaFromSurvey(aid) {
  ...
  const name = anketa ? anketa.name : 'анкету';
  document.getElementById('modal-confirm-title').textContent = 'Убрать анкету?';
  document.getElementById('modal-confirm-text').textContent  = `Убрать «${name}» из опроса? Это действие нельзя отменить.`;
  _confirmCallback = () => {
    s.anketaIds = (s.anketaIds || []).filter(x => x !== aid);
    _selectedAnketaIds.delete(aid);
    Store.surveys = surveys;
    closeModal('modal-confirm');
    renderSurveyOpros(s);
...
function toggleAnketaCheck(aid, checked) {
  if (checked) _selectedAnketaIds.add(aid); else _selectedAnketaIds.delete(aid);
  const btn = document.getElementById('bulk-remove-anketa');
  if (btn) {
    btn.style.display = _selectedAnketaIds.size ? '' : 'none';
    btn.textContent = `Убрать выбранные (${_selectedAnketaIds.size})`;
  }
}
function toggleProgramCheck(pid, checked) {
  if (checked) _selectedProgramIds.add(pid); else _selectedProgramIds.delete(pid);
  const btn = document.getElementById('bulk-remove-prog');
  if (btn) {
    btn.style.display = _selectedProgramIds.size ? '' : 'none';
    btn.textContent = `Убрать выбранные (${_selectedProgramIds.size})`;
  }
}
function bulkRemoveAnkety() {
  if (!_selectedAnketaIds.size) return;
  const count = _selectedAnketaIds.size;
  const noun = count === 1 ? 'анкету' : count < 5 ? 'анкеты' : 'анкет';
  document.getElementById('modal-confirm-title').textContent = 'Убрать анкеты?';
  document.getElementById('modal-confirm-text').textContent  = `Убрать ${count} ${noun} из опроса? Это действие нельзя отменить.`;
  _confirmCallback = () => {
    const surveys = Store.surveys;
    const s = surveys.find(x => x.id === currentSurveyId);
    if (!s) return;
    s.anketaIds = (s.anketaIds || []).filter(x => !_selectedAnketaIds.has(x));
    _selectedAnketaIds.clear();
    Store.surveys = surveys;
    closeModal('modal-confirm');
    renderSurveyOpros(s);
    toast('Анкеты убраны из опроса');
  };
  openModal('modal-confirm');
}
function bulkRemovePrograms() {
  if (!_selectedProgramIds.size) return;
  const count = _selectedProgramIds.size;
  const noun = count === 1 ? 'программу' : count < 5 ? 'программы' : 'программ';
  document.getElementById('modal-confirm-title').textContent = 'Убрать программы?';
  document.getElementById('modal-confirm-text').textContent  = `Убрать ${count} ${noun} из опроса? Это действие нельзя отменить.`;
  _confirmCallback = () => {
    const surveys = Store.surveys;
    const s = surveys.find(x => x.id === currentSurveyId);
    if (!s) return;
    s.programIds = (s.programIds || []).filter(x => !_selectedProgramIds.has(x));
    _selectedProgramIds.clear();
    Store.surveys = surveys;
    closeModal('modal-confirm');
    renderSurveyOpros(s);
    ...
    toast('Программы убраны из опроса');
  };
  openModal('modal-confirm');
}
...
function removeProgramFromSurvey(pid) {
  ...
  const name = prog ? prog.name : 'программу';
  document.getElementById('modal-confirm-title').textContent = 'Убрать программу?';
  document.getElementById('modal-confirm-text').textContent  = `Убрать «${name}» из опроса? Это действие нельзя отменить.`;
  _confirmCallback = () => {
    s.programIds = (s.programIds || []).filter(x => x !== pid);
    Store.surveys = surveys;
    closeModal('modal-confirm');
    renderSurveyOpros(s);
    if (currentSurveyTab === 'nastroyki') renderSurveyNastroyki(s);
...
async function saveAnketa() {
  ...
  else ankety.push(_editingAnketa);
  localStorage.setItem('sib_ankety', JSON.stringify(ankety));
  if (_IS_SERVER) {
    try {
      const r = await fetch('/api/data/ankety', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ value: ankety }),
      });
      if (r.status === 401) { showLoginOverlay(); return; }
      if (!r.ok) {
        const errText = await r.text().catch(() => r.status);
        console.error('saveAnketa error:', errText);
        toast('Ошибка сохранения: ' + errText);
        return;
      }
    } catch(e) {
      console.error('saveAnketa fetch error:', e);
      toast('Ошибка сохранения: нет связи с сервером');
      return;
    }
  }
  _hasUnsavedChanges = false;
...
let _reportProgramFilter = {};
function calcSurveyAvgCoeff(survey, responses) {
  const ankety = (survey.anketaIds || []).map(aid => Store.ankety.find(a => a.id === aid)).filter(Boolean);
  let totalScore = 0, scoredCount = 0;
  ankety.forEach(anketa => {
    const aResps = responses.filter(r => r.anketaId === anketa.id);
    if (!aResps.length) return;
    anketa.questions.forEach(q => {
      if (q.type !== 'radio' || !q.options?.length) return;
      const counts = {};
      ...
      let wSum = 0, wTotal = 0;
      q.options.forEach((o, oi) => { const pts = Math.max(1, 5 - oi); wSum += pts * counts[o]; wTotal += counts[o]; });
      if (wTotal > 0) { totalScore += wSum / wTotal; scoredCount++; }
    });
  });
  return scoredCount > 0 ? (totalScore / scoredCount).toFixed(1) : null;
}
function renderReports() {
  ...
  cont.innerHTML = surveys.map(s => {
    ...
    const passCount = countSurveyRespondents(s.id);
    const statusLabel = STATUS_LABELS[s.status] || s.status;
    ...
    const sProgramMap = new Map();
    Store.surveyLinks.filter(l => l.surveyId === s.id).forEach(l => {
      if (l.programId && !sProgramMap.has(l.programId)) {
        const prog = Store.programs.find(p => p.id === l.programId);
        sProgramMap.set(l.programId, prog || { id: l.programId, name: l.programName || l.programId, code: '' });
      }
    });
    const sPrograms = [...sProgramMap.values()];
    const avgCoeff = calcSurveyAvgCoeff(s, responses);
    return `<div class="detail-card" style="margin-bottom:20px">
      ...
      <div id="report-body-${s.id}" data-open="0" style="max-height:0;overflow:hidden;opacity:0;transition:max-height .3s ease,opacity .25s ease">
        <div style="padding-top:12px">
          ${avgCoeff !== null ? `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#f0f7ff;border-radius:8px;margin-bottom:12px">
            <span style="font-size:13px;color:#424242">Средний коэффициент удовлетворённости</span>
            <span style="font-size:20px;font-weight:700;color:#0078ff">${avgCoeff}</span>
          </div>` : ''}
          ${sPrograms.length > 0
            ? `<div style="display:flex;align-items:center;gap:6px;padding:6px 0 2px;border-top:1px solid #f0f0f0">
                <input type="checkbox" id="prog-all-${s.id}" onchange="toggleAllProgChecks('${s.id}', this.checked)" style="cursor:pointer">
                <label for="prog-all-${s.id}" style="font-size:12px;color:#757575;cursor:pointer">Выбрать все программы</label>
              </div>`
            : ''}
          ${sPrograms.length > 0
            ? sPrograms.map(p => {
                const pbId = 'pb-' + s.id + '-' + p.id;
                const pLinks = Store.surveyLinks.filter(l => l.surveyId === s.id && l.programId === p.id);
                const pLinkIds = new Set(pLinks.map(l => l.id));
                const pResps = responses.filter(r => pLinkIds.has(r.linkId));
                const pCount = pLinks.reduce((sum, l) => sum + (l.usedCount || 0), 0);
                return `<div style="border-top:1px solid #f0f0f0">
                  <div onclick="toggleProgBody('${pbId}')" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;padding:10px 0;user-select:none">
                    <div style="display:flex;align-items:center;gap:8px;flex:1;min-width:0">
                      <input type="checkbox" class="prog-check" data-survey-id="${s.id}" data-prog-id="${p.id}" onclick="event.stopPropagation()" style="flex-shrink:0;cursor:pointer">
                      <div style="min-width:0">
                        <div style="font-size:13px;font-weight:600;color:#424242">${p.code ? escHtml(p.code) + ' — ' : ''}${escHtml(p.name)}</div>
                        ${p.programName ? `<div style="font-size:11px;color:#9e9e9e;margin-top:1px">${escHtml(p.programName)}</div>` : ''}
                      </div>
                    </div>
                    <div style="display:flex;align-items:center;gap:12px;flex-shrink:0">
                      <div style="text-align:right">
                        <div style="font-size:28px;font-weight:700;color:#0078ff;line-height:1">${pCount}</div>
                        <div style="font-size:11px;color:#9e9e9e">ответов</div>
                      </div>
                      <svg id="chev-${pbId}" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#bdbdbd" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="transition:transform .25s"><polyline points="6 9 12 15 18 9"/></svg>
                    </div>
                  </div>
                  <div id="${pbId}" data-open="0" style="max-height:0;overflow:hidden;opacity:0;transition:max-height .35s ease,opacity .25s ease">
                    <div style="padding-bottom:12px">
                      ${pCount ? buildResponseStats(s, pResps) : '<div style="color:#9e9e9e;font-size:12px;padding:4px 0 8px">Нет ответов по этой программе</div>'}
                    </div>
                  </div>
                </div>`;
              }).join('')
            : !responses.length
              ? `<div style="color:#9e9e9e;font-size:13px;padding:12px 0">Ответов пока нет</div>`
              : buildResponseStats(s, responses)
          }
        </div>
      </div>
    </div>`;
...
function toggleReportBody(id) {
  ...
  if (open) {
    ...
    body.dataset.open = '1';
    if (chev) chev.style.transform = 'rotate(180deg)';
    setTimeout(() => { if (body.dataset.open === '1') body.style.maxHeight = 'none'; }, 350);
  }
}
function exportReport(surveyId, format, filterProgramId) {
  const s = Store.surveys.find(x => x.id === surveyId);
  if (!s) return;
  const allLinks = Store.surveyLinks.filter(l => l.surveyId === surveyId);
  const filteredLinks = filterProgramId ? allLinks.filter(l => l.programId === filterProgramId) : allLinks;
  const activeLinkIds = new Set(filteredLinks.map(l => l.id));
  ...
  const programs = filterProgramId
    ? (s.programIds || []).map(pid => Store.programs.find(p => p.id === pid)).filter(p => p && p.id === filterProgramId)
    : (s.programIds || []).map(pid => Store.programs.find(p => p.id === pid)).filter(Boolean);
  const year = s.dateEnd ? new Date(s.dateEnd).getFullYear() : new Date().getFullYear();
  const categoryMap = { 'Обучающиеся':'обучающихся','Преподаватели':'преподавателей','Сотрудники':'сотрудников','Гости':'гостей','Работодатели':'работодателей' };
  ...
  const programTitle = multiplePrograms
    ...
    : mainProgram ? `«${escHtml(mainProgram.programName || mainProgram.name)}»` : `«${escHtml(s.title)}»`;
  const programCode = multiplePrograms
    ? ''
    : mainProgram ? `(${escHtml(mainProgram.code)} ${escHtml(mainProgram.name)})` : '';
  const programsListHtml = multiplePrograms
    ? `<p style="font-size:14pt;line-height:1.6">${programs.map(p => `${escHtml(p.code)} ${escHtml(p.name)}${p.programName ? ' — ' + escHtml(p.programName) : ''}`).join(',<br>')}</p>`
    : '';
  const allQuestions = [];
  ankety.forEach(anketa => {
    anketa.questions.forEach(q => {
  ...
  const resultsRows = allQuestions.map(({ q, responses }, i) => {
    ...
    if (q.type === 'radio') {
      ...
      if (responses.length > 0) {
        ...
        statsHtml = q.options.map(o => {
          const cnt = counts[o] || 0;
          const pct = total > 0 ? Math.round(cnt / total * 100) : 0;
          return `${escHtml(o)}: ${cnt} (${pct}%)`;
        }).join('<br>');
      }
      return `<tr><td>${i+1}</td><td>${escHtml(q.text)}${q.required?'*':''}</td><td>${statsHtml}</td></tr>`;
    }
    return null;
  }).filter(Boolean).join('');
  ...
  const subtitleHtml = (!filterProgramId || multiplePrograms)
    ? `о результатах&nbsp;проведения анкетирования ${categoryGen}<br>«${escHtml(s.title)}»`
    : `о результатах&nbsp;проведения анкетирования ${categoryGen}<br>«${escHtml(s.title)}»<br>по образовательной программе<br>${programTitle}<br>${programCode}`;
  ...
  *{box-sizing:border-box}
  ...
  <table class="data-table"><colgroup><col class="num"><col><col class="score"></colgroup>
    <thead><tr><th>№ п/п</th><th>Вопрос</th><th>Показатель удовлетворенности / Распределение ответов</th></tr></thead>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" style="width:100%;border:none;border-collapse:collapse;page-break-after:always">
  ...
  <tr height="400" style="height:400pt;mso-height-rule:exactly">
    <td align="center" valign="middle" style="border:none;padding:0;text-align:center;vertical-align:middle">
      ...
      <p style="font-family:'Times New Roman',serif;font-size:14pt;line-height:1.8;margin:0;text-align:center">«${escHtml(s.title)}»</p>
      ${(!filterProgramId || multiplePrograms)
        ? ''
...
<style>${commonStyles}
  @page WordSection1{size:21.0cm 29.7cm;margin:25mm 25mm 25mm 25mm;mso-page-orientation:portrait}
  div.WordSection1{page:WordSection1}
  body{margin:0;padding:0;width:16cm}
</style></head><body>
...
<div style="padding-top:20mm">${contentHtml}</div>
    ...
    a.href = url;
    const _slug = t => t.replace(/[^а-яёА-ЯЁa-zA-Z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
    const _progSuffix = mainProgram ? '_' + _slug((mainProgram.code ? mainProgram.code + '_' : '') + mainProgram.name) : '';
    a.download = `Отчет_${_slug(s.title)}${_progSuffix}.doc`;
    a.click();
...
<div style="width:210mm;min-height:297mm;padding:25mm 25mm 20mm;page-break-after:always;display:flex;flex-direction:column;justify-content:space-between;align-items:center;text-align:center">
  ...
  <div style="font-family:'Times New Roman',serif;font-size:13pt;line-height:1.8">
    ...
    const _slugPdf = t => t.replace(/[^а-яёА-ЯЁa-zA-Z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
    const pdfTitle = `Отчет_${_slugPdf(s.title)}${mainProgram ? '_' + _slugPdf((mainProgram.code ? mainProgram.code + '_' : '') + mainProgram.name) : ''}`;
    const pdfHtml = `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>${pdfTitle}</title><style>${commonStyles}
    ...
    anketaIds.map(aid => {
      ...
      const stats = anketa.questions.map(q => {
        if (q.type === 'radio' || q.type === 'checkbox') {
          ...
          const coeff = (q.type === 'radio' && wTotal > 0) ? (wSum / wTotal) : null;
...
function renderStudentSurvey() {
  ...
  if (activeLinkId && hasDeviceUsedLink(activeLinkId)) {
    ...
    wrap.innerHTML = `
      <div style="text-align:center;padding:20px;min-height:60vh;display:flex;flex-direction:column;align-items:center;justify-content:center">
        <div style="font-size:22px;font-weight:700;color:#212121;margin-bottom:10px">Опрос уже пройден</div>
        <div style="font-size:15px;color:#757575">Вы уже проходили опрос по этой ссылке с данного устройства</div>
      </div>`;
    return;
  }
  if (!survey) {
  ...
  const submitHtml = ankety.length
    ? `<button class="btn btn-blue" style="width:100%;padding:14px;font-size:15px;margin-top:8px;margin-bottom:32px" onclick="submitStudentSurvey()">Отправить ответы</button>`
    : '';
  wrap.innerHTML = `
...
function submitStudentSurvey() {
  ...
  const ankety = (survey.anketaIds || [])
    ...
    .filter(Boolean);
  if (!ankety.length) {
    toast('Прохождение недоступно: к опросу не прикреплена анкета');
    return;
  }
  let firstError = null;
  const newResponses = [];
  ...
  if (activeLinkId && hasDeviceUsedLink(activeLinkId)) {
    toast('Вы уже проходили опрос по этой ссылке');
    sessionStorage.removeItem('sibgiu_active_link');
    renderThankYou();
    return;
  }
  ...
  if (activeLinkId) {
    markDeviceUsedLink(activeLinkId);
...
function _ensureSurveyLinks(surveyId) {
  ...
  if (isStudents) {
    const allStudentGroups = Store._get('studentGroups') || [];
    for (const prog of programs) {
      // Match strictly by profile (programName), then by code
      const targetProfile = prog.programName || prog.name;
      const progGroups = allStudentGroups.filter(g => {
        if (g.programName !== targetProfile) return false;
        if (g.code && prog.code) return g.code === prog.code;
        return true;
      });
      // Remove links for groups that no longer belong to this program
      const validGroupNames = new Set(progGroups.map(g => g.name));
      const beforeGroupClean = links.length;
      links = links.filter(l =>
        !(l.surveyId === surveyId && l.programId === prog.id && l.groupName && !validGroupNames.has(l.groupName))
      );
      if (links.length !== beforeGroupClean) changed = true;
...
function getSurveyLinkUrl(linkId) {
  return window.location.href.split('#')[0] + '#survey=' + linkId;
}
...
function copyAccessLinkValue(btn) {
  const input = btn.closest('.access-link-url-row')?.querySelector('.access-link-url');
  ...
  copyTextToClipboard(text)
    ...
    .catch(() => {
      input.select();
      toast('Не удалось скопировать — выделите ссылку и нажмите Ctrl+C');
    });
}
function copySelectedAccessLinksText() {
  const ta = document.getElementById('access-selected-links');
  if (!ta?.value) { toast('Выберите хотя бы одну ссылку'); return; }
  copyTextToClipboard(ta.value)
    .then(() => toast('Ссылки скопированы'))
    .catch(() => {
      ta.select();
      toast('Не удалось скопировать — выделите текст и нажмите Ctrl+C');
    });
}
function accessLinkUrlRow(linkId) {
  const url = getSurveyLinkUrl(linkId);
  const widthCh = Math.min(Math.max(url.length + 1, 28), 72);
  return `<div class="access-link-url-row">
    <button type="button" class="btn-copy-link" onclick="copyAccessLinkValue(this)">Скопировать</button>
    <input type="text" class="access-link-url" readonly value="${escHtml(url)}" style="width:${widthCh}ch" onclick="this.select()" title="Нажмите, чтобы выделить, затем Ctrl+C">
  </div>`;
}
function updateSelectedAccessLinks() {
  const wrap = document.getElementById('access-selected-wrap');
  const ta = document.getElementById('access-selected-links');
  if (!wrap || !ta) return;
  ...
  ta.value = checked.map(id => getSurveyLinkUrl(id)).join('\n');
...
function renderSurveyAccess(surveyId) {
  ...
  if (!cont || !s) return;
  const aids = (s.anketaIds || []).filter(aid => Store.ankety.some(a => a.id === aid));
  if (!aids.length) {
    cont.innerHTML = `<div class="empty-state"><p>Добавьте анкеты во вкладке «Опрос»</p></div>`;
    return;
  }
  ...
  if (isStudents) {
    contentHtml = programs.map(prog => {
        ...
        <div class="access-prog-block">
          <div class="access-prog-header" onclick="toggleAccessProg(this)">
            ...
            <div>
              ...
              ${prog.programName ? `<div style="font-size:11px;color:#9e9e9e;margin-top:1px">${escHtml(prog.programName)}</div>` : ''}
            </div>
      ...
      const groupRows = progLinks.map(l => {
        ...
        <div class="access-link-row${exhausted ? ' access-link-exhausted' : ''}">
          <div class="access-link-main">
            <input type="checkbox" class="access-check" data-link-id="${l.id}"${exhausted ? ' disabled' : ''} onchange="updateSelectedAccessLinks()">
            ...
            ${exhausted ? `<span class="access-exhausted-msg">Лимит исчерпан</span>` : ''}
          </div>
          ${exhausted ? '' : accessLinkUrlRow(l.id)}
        </div>`;
      }).join('');
    ...
    contentHtml = programs.map(prog => {
      ...
      return `
        <div class="access-link-row${exhausted ? ' access-link-exhausted' : ''}">
          <div class="access-link-main">
            <input type="checkbox" class="access-check" data-link-id="${link.id}"${exhausted ? ' disabled' : ''} onchange="updateSelectedAccessLinks()">
            <div class="access-prog-info">
          ...
          ${exhausted ? '' : accessLinkUrlRow(link.id)}
        </div>`;
    }).join('');
  }
  ...
  cont.innerHTML = `
    ...
    <div class="access-selected-wrap" id="access-selected-wrap" style="display:none">
      <div class="access-selected-head">
        <span class="access-selected-label">Выбранные ссылки</span>
        <button type="button" class="btn-copy-link" onclick="copySelectedAccessLinksText()">Скопировать</button>
      </div>
      <textarea readonly class="access-selected-links" id="access-selected-links" onclick="this.select()"></textarea>
    </div>
...
function _previewGroupsRows(rows) {
  ...
  const normalized = rows.map(r => {
    const profKey = Object.keys(r).find(k => k.toLowerCase().includes('профил'));
    ...
    // Prefer profile over direction as the unique program key
    const rawProg = String((profKey ? r[profKey] : null) ?? (dirKey ? r[dirKey] : null) ?? _findCol(r, 'программ', 'program') ?? '').trim();
  ...
  // Use compound key "code|prog" so programs with same profile but different codes stay separate
  const byProg = {};
  for (const r of normalized) {
    const key = (r.code ? r.code + '|' : '') + r.prog;
    if (!byProg[key]) byProg[key] = { code: r.code || '', prog: r.prog, groups: [] };
    const existing = byProg[key].groups.find(g => g.name === r.group);
    if (existing) { existing.count = r.count; }
    else byProg[key].groups.push({ name: r.group, count: r.count });
  }
  _parsedGroupsData = byProg;
  const previewHtml = Object.values(byProg).map(({ code, prog, groups }) => `
...
function saveGroupsFromParsed() {
  ...
  // Store groups directly with code for precise per-program lookup
  const existingArr = Store._get('studentGroups') || [];
  const importedEntries = Object.values(_parsedGroupsData);
  // Remove ALL old records whose programName matches any imported entry (old records have no code)
  const importedProgNames = new Set(importedEntries.map(d => d.prog));
  const kept = existingArr.filter(x => !importedProgNames.has(x.programName));
  const newEntries = [];
  for (const { code, prog, groups } of importedEntries) {
    for (const g of groups) {
      const found = existingArr.find(x => x.programName === prog && x.name === g.name && (!x.code || x.code === code));
      newEntries.push({ id: found ? found.id : uid(), code: code || '', programName: prog, name: g.name, count: g.count });
    }
  }
  const merged = [...kept, ...newEntries];
  localStorage.setItem('sib_studentGroups', JSON.stringify(merged));
  _syncToServer('studentGroups', merged);
  const programs = Store.programs;
  for (const { code, prog } of importedEntries) {
    if (code) {
      const existingProg = programs.find(p => p.code === code && (p.programName === prog || p.name === prog));
      if (!existingProg) {
        programs.unshift({ id: uid(), code, name: prog, programName: prog, dateModified: Date.now() });
      }
    }
  }
  Store.programs = programs;
  const links = Store.surveyLinks;
  let changed = false;
  for (const { prog, groups } of importedEntries) {
    for (const g of groups) {
      const link = links.find(l => l.programName === prog && l.groupName === g.name);
      if (link && link.limit !== g.count) { link.limit = g.count; changed = true; }
    }
  }
  if (changed) Store.surveyLinks = links;
...
function renderSurveyStat(surveyId) {
  ...
  const ankety = (s.anketaIds || []).map(aid => Store.ankety.find(a => a.id === aid)).filter(Boolean);
  const totalHtml = (s.programIds || []).length
    ? ''
    : `<div class="empty-state"><p>Ответов пока нет</p></div>`;
  let groupsHtml = '';
  if (links.length) {
...
function _autoCompleteExpiredSurveys() {
  const today = new Date().toISOString().slice(0, 10);
  const surveys = Store.surveys;
  let changed = false;
  for (const s of surveys) {
    if (s.status === 'active' && s.dateEnd && s.dateEnd < today) {
      s.status = 'completed';
      changed = true;
    }
  }
  if (changed) Store.surveys = surveys;
}
async function init() {
  await _loadFromServer();
  ensurePrograms();
  _autoCompleteExpiredSurveys();
  const hash = window.location.hash;
  if (hash.startsWith('#survey=')) {
    const linkId = hash.slice(8);

    // Determine if this is an admin session (same tab or new tab)
    let isAdmin = sessionStorage.getItem('sibgiu_auth') === 'admin';
    if (!isAdmin && _IS_SERVER) {
      try {
    ...
    if (isAdmin) {
      // Admin previewing survey — use local Store, don't touch the server session
      const link = Store.surveyLinks.find(l => l.id === linkId);
      if (!link) {
        showSurveyLinkMessage('Ссылка недействительна', 'Ссылка не найдена');
        return;
      }
      const survey = Store.surveys.find(s => s.id === link.surveyId);
      const aids = (survey?.anketaIds || []).filter(aid => Store.ankety.some(a => a.id === aid));
      if (!aids.length) {
        showSurveyLinkMessage('Ссылка недействительна', 'К опросу не прикреплена анкета');
        return;
      }
    ...
    if (hasDeviceUsedLink(linkId)) {
      showSurveyLinkMessage('Опрос уже пройден', 'Вы уже проходили опрос по этой ссылке с данного устройства');
      return;
    }
    try {
      const r = await fetch('/api/link-auth', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!r.ok) {
        const msg = r.status === 403 ? 'Лимит прохождений исчерпан' : (data.error || 'Ссылка не существует');
        showSurveyLinkMessage('Ссылка недействительна', msg);
        return;
      }
      const survey = Store.surveys.find(s => s.id === data.surveyId);
      const aids = (survey?.anketaIds || []).filter(aid => Store.ankety.some(a => a.id === aid));
      if (!aids.length) {
        showSurveyLinkMessage('Ссылка недействительна', 'К опросу не прикреплена анкета');
        return;
      ...
      hideLoginOverlay();
      openStudentSurvey(data.surveyId);
    } catch(e) {
      showSurveyLinkMessage('Ошибка', 'Не удалось проверить ссылку. Попробуйте позже.');
    }
    return;
  }
  const auth = sessionStorage.getItem('sibgiu_auth');
...
function toggleSurveyProgChecks(surveyId, checked) {
  document.querySelectorAll(`.prog-check[data-survey-id="${surveyId}"]`).forEach(cb => cb.checked = checked);
  updateReportSelection();
}
function toggleAllReportChecks(master) {
  const wantChecked = master.checked;
  document.querySelectorAll('.report-check').forEach(cb => {
    cb.checked = wantChecked;
    document.querySelectorAll(`.prog-check[data-survey-id="${cb.dataset.id}"]`).forEach(p => p.checked = wantChecked);
  });
  updateReportSelection();
}
  ...
  const surveyBlocks = allSurveys.map(s => {
        ...
        const qBlocks = anketa.questions.map(q => {
          if (q.type === 'radio' || q.type === 'checkbox') {
            ...
            const rows = Object.entries(counts).map(([opt,cnt]) => {
              ...
              if (forWord) {
                return `<tr><td style="padding:4px 8px;font-size:12px">${escHtml(opt)}</td><td style="padding:4px 8px;font-size:12px;color:#0078ff;font-weight:600">${cnt}</td><td style="padding:4px 8px;font-size:12px;color:#9e9e9e">${pct}%</td></tr>`;
              }
              return `<div style="margin-bottom:8px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px"><span>${escHtml(opt)}</span><span style="font-weight:600;color:#0078ff">${cnt} (${pct}%)</span></div><div style="height:8px;background:#e0efff;border-radius:4px"><div style="height:100%;width:${pct}%;background:#0078ff;border-radius:4px"></div></div></div>`;
            }).join('');
            if (forWord) {
              return `<p style="font-size:13px;font-weight:600;margin:10px 0 4px">${escHtml(q.text)}</p><table border="1" cellspacing="0" style="border-collapse:collapse;width:100%;font-family:Arial">${rows}</table>`;
            }
            return `<div style="margin-bottom:16px"><div style="font-size:13px;font-weight:600;margin-bottom:8px">${escHtml(q.text)}</div>${rows}</div>`;
          } else {
            const answers = aResps.map(r => r.answers[q.id]).filter(Boolean);
            if (!answers.length) return '';
            const ansHtml = answers.map(a => `<li style="font-size:12px;color:#424242;margin-bottom:3px">${escHtml(String(a))}</li>`).join('');
            return `<div style="margin-bottom:16px"><div style="font-size:13px;font-weight:600;margin-bottom:6px">${escHtml(q.text)}</div><ul style="margin:0;padding-left:16px">${ansHtml}</ul></div>`;
          }
        }).filter(Boolean).join('');
        return `<div style="margin-bottom:16px;padding:12px;background:#f7f9ff;border-radius:8px"><div style="font-size:13px;font-weight:700;color:#0078ff;margin-bottom:10px">${escHtml(anketa.name)} — ${aResps.length} ответов</div>${qBlocks}</div>`;
      }).join('');
    })();
    return `<div style="margin-bottom:28px;padding:20px;border:1.5px solid #e0efff;border-radius:12px;page-break-inside:avoid">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
        <div>
          <div style="font-size:16px;font-weight:700;color:#212121;margin-bottom:4px">${escHtml(s.title)}</div>
          ${anketaNames.length ? `<div style="font-size:12px;color:#757575">Анкеты: ${anketaNames.join(', ')}</div>` : ''}
          <div style="font-size:12px;color:#9e9e9e;margin-top:2px">${fmtDateRange(s.dateStart, s.dateEnd)} · Статус: ${escHtml(statusLabel)}</div>
        </div>
...
function toggleAllProgChecks(surveyId, checked) {
  document.querySelectorAll(`.prog-check[data-survey-id="${surveyId}"]`).forEach(cb => cb.checked = checked);
}
function _getExportTargets() {
  const targets = [];
  [...document.querySelectorAll('.report-check:checked')].forEach(cb => {
    targets.push({ surveyId: cb.dataset.id, progId: null });
  });
  [...document.querySelectorAll('.prog-check:checked')].forEach(cb => {
    targets.push({ surveyId: cb.dataset.surveyId, progId: cb.dataset.progId });
  });
  return targets;
}
async function exportReportPDF() {
  const targets = _getExportTargets();
  if (!targets.length) { toast('Выберите хотя бы один опрос или программу'); return; }
  if (targets.length > 1) toast(`Скачивание ${targets.length} файлов, подождите...`);
  for (let i = 0; i < targets.length; i++) {
    exportReport(targets[i].surveyId, 'pdf', targets[i].progId);
...
async function exportReportWord() {
  ...
  if (targets.length > 1) toast(`Скачивание ${targets.length} файлов, подождите...`);
  for (let i = 0; i < targets.length; i++) {
    exportReport(targets[i].surveyId, 'word', targets[i].progId);
    if (i < targets.length - 1) await new Promise(r => setTimeout(r, 600));
  }
}