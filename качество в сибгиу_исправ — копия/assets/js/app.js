...
function doLogin() {
  ...
  if (token) {
    const survey = Store.surveys.find(s => s.id === token.surveyId);
    const aids = (survey?.anketaIds || []).filter(aid => Store.ankety.some(a => a.id === aid));
    if (!aids.length) {
      errEl.textContent = 'Прохождение недоступно: к опросу не прикреплена анкета';
...
async function init() {
  ...
  if (hash.startsWith('#survey=')) {
    ...
    const link = Store.surveyLinks.find(l => l.id === linkId);
    if (link && hasDeviceUsedLink(linkId)) {
    ...
    showSurveyLinkMessage('Ссылка недействительна', 'Лимит прохождений исчерпан или ссылка не существует');
    return;
  }
  const auth = sessionStorage.getItem('sibgiu_auth');
  if (auth === 'admin') {
