    ...
    return `<div class="detail-card" style="margin-bottom:20px">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;cursor:pointer;user-select:none" onclick="toggleReportBody('${s.id}')">
        <div style="display:flex;align-items:flex-start;gap:10px;flex:1;min-width:0">
          <input type="checkbox" class="report-check" data-id="${s.id}" onchange="toggleSurveyProgChecks('${s.id}', this.checked)" onclick="event.stopPropagation()" style="margin-top:4px;flex-shrink:0;cursor:pointer">
...
<div style="padding-top:20mm">${contentHtml}</div>
    ...
    const _progSuffix = mainProgram ? '_' + _slug(mainProgram.code || mainProgram.name) : '';
...
<div style="padding:20mm 25mm 20mm">${contentHtml}</div>
    ...
    iframe.contentDocument.title = `Отчет_${_slugPdf(s.title)}${mainProgram ? '_' + _slugPdf(mainProgram.code || mainProgram.name) : ''}`;
    setTimeout(() => {
...
function _getExportTargets() {
  const progChecked = [...document.querySelectorAll('.prog-check:checked')];
  if (progChecked.length > 0) {
    return progChecked.map(cb => ({ surveyId: cb.dataset.surveyId, progId: cb.dataset.progId }));
  }
  return getSelectedSurveyIds().map(id => ({ surveyId: id, progId: null }));
}
